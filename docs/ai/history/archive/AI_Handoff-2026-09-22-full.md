# Velnox Handoff Archive — history only

> **Reference only — do not load automatically.** The current state lives in
> [`AI_Handoff.md`](./AI_Handoff.md); load that first.
>
> Everything below is a chronological record of completed, superseded or
> investigated work. Nothing here is guaranteed to describe the current code —
> the repository is always authoritative. Entries are oldest-first.

> **Current state has moved.** The architecture, the V / verification model, the
> review vocabulary, the review-history contract and the verification API surface
> are documented in [`AI_Handoff.md`](./AI_Handoff.md). Everything from here down
> is dated history.
