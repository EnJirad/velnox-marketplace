 ...
 *
 * All logic that the Auth page needs beyond calling the auth backend lives here so
 * it can be tested without a browser or a backend. These helpers only shape UX (normalization,
 * masking, friendly error selection, resend countdown). Nothing here can
 * bypass a server-side check.
 */

/** Must match the backend OTP length (6 digits). */
export const OTP_LENGTH = 6;

/** Client-side mirror of the backend OTP lifetime. */
 ...
 * Auth reports every verification failure as "Could not verify code",
...
/**
 * True when an OAuth `code` is currently in the URL (the auth client
