/**
 * Velnox — Customer Memory Core (pure logic)
 *
 * The deterministic heart of the Customer Memory engine. Everything here is
 * pure TypeScript so it can be unit-tested and reused by the API layer.
 *
 * Pipeline (CPNS §6):
 ...
 * Pure + deterministic → unit-tested; the API mutation applies the plan.
 */
