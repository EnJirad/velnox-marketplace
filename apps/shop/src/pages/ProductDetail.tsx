...
const demoProduct: Product = {
  ...
  description: "This hand-crafted ceramic vase features a clean, modern silhouette that complements any interior. Made by skilled artisans using traditional techniques, each piece is unique with subtle variations in glaze and form.\n\nPerfect for dried flowers, branches, or simply as a standalone decorative piece. The matte finish and organic shape bring warmth and sophistication to your space.",
  shortDescription: "Hand-crafted modern vase", price: 1290, compareAtPrice: 1590,
  currency: "THB", status: "active", featured: true, rating: 4.8, reviewCount: 124,
  soldCount: 342, categoryId: "c1",
  shop: { id: "s1", sellerId: "sel1", name: "Artisan Studio", slug: "artisan-studio",
    description: "Handmade ceramics and home goods", logo: null, cover: null,
    rating: 4.9, productCount: 48, createdAt: "" },
  images: [
    { id: "i1", productId: "1", url: "", alt: "Ceramic Vase - Front", sortOrder: 0 },
    { id: "i2", productId: "1", url: "", alt: "Ceramic Vase - Side", sortOrder: 1 },
  ],
...
export default function ProductDetail() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const { product: p } = await productsApi.get(id!);
  ...
  const handleAddToCart = async () => {
    ...
    try {
      ...
      toast.error("Could not add to cart", { description: "Please sign in first." });
    }
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
          ...
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.4 }}>
            <div className="aspect-square rounded-2xl bg-muted overflow-hidden mb-3">
              {p.images[selectedImage]?.url ? (
                <img src={p.images[selectedImage].url} alt={p.images[selectedImage].alt} className="h-full w-full object-cover" />
              ) : (
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.4 }} className="space-y-6">
            ...
            <div className="flex items-center gap-3">
              {p.shop && (
                <Link to={`/shops/${p.shop.slug}`} className="text-sm text-muted-foreground hover:text-foreground transition-colors font-medium">
                  {p.shop.name}
                </Link>
              )}
              ...
              <div className="flex items-center gap-2">
                <div className="flex items-center gap-1">
                  {Array.from({ length: 5 }).map((_, i) => (
                    <Star key={i} className={`h-4 w-4 ${i < Math.round(p.rating!) ? "fill-amber-400 text-amber-400" : "text-muted"}`} />
                  ))}
                </div>
                ...
                <span className="text-sm text-muted-foreground">{p.soldCount} sold</span>
              </div>
            )}
            ...
            <div className="flex items-baseline gap-3">
              ...
              {hasDiscount && (
                <>
                  <span className="text-lg text-muted-foreground line-through">{formatPrice(p.compareAtPrice!, p.currency)}</span>
                  <Badge variant="destructive" className="text-xs">
                    Save {formatPrice(p.compareAtPrice! - p.price, p.currency)}
                  </Badge>
                </>
              )}
            </div>
            ...
            {p.shop && (
              <div className="rounded-xl border border-border/60 p-4 flex items-center gap-4">
                ...
                <div className="flex-1">
                  <p className="text-sm font-medium">{p.shop.name}</p>
                  <p className="text-xs text-muted-foreground">{p.shop.productCount} products · {p.shop.rating?.toFixed(1)} rating</p>
                </div>
